...
const Directory = ({ data }) => {
    ...
    async function updateSearchResults() {
      ...
      orFilters.forEach(filter => {
        ...
        filter.results.forEach(filterRes => {
          if (
            !finalResults.find(
              finalRes => finalRes.node.name === filterRes.node.name
            )
          ) {
      ...
      andFilters.forEach(filter => {
        ...
        finalResults = finalResults.filter(finalRes =>
          filter.results.find(
            filterRes => finalRes.node.name === filterRes.node.name
          )
        )
      })
        ...
        if (filter.type === "checkbox") {
          ...
          filter.results = searchList.filter(
            ({ node }) =>
              ...
              trueVals[0] &&
              trueVals.some(val =>
                node[filter.field].toLowerCase().includes(val.toLowerCase())
              )
          )
        } else if (filter.type === "fuzzy") {
          ...
          filter.results = searchList.filter(
            ({ node }) =>
              ...
              node[filter.field]
                .toLowerCase()
                .includes(formik.values[filter.field].trim().toLowerCase())
          )
        }
      } else {
        filter.results = []
  ...
  return (
    ...
    artists."
      footerQuoteText={<blockquote>"You have to see it to be it."</blockquote>}
    ...
    >
      <section className="section_intro">
        <h1>Directory</h1>
        <p>
          We have curated a directory of actors, directors, dancers, singers,
          stage managers, lighting designers, dramaturgs, artistic directors,
          producers, casting directors, librettists, lyricists, composers, props
          designers, scenic designers, sound designers, choreographers, costume
          designers, and production managers who self-identify as lesbian,
          bisexual, trans, queer, femme, masc, non-binary, and the diversity of
          genders that queerness contains.
        </p>
      </section>
      ...
      <section id="key__grid" className="key__grid">
        {searchResults[0] ? (
          searchResults.map((obj, i, arr) => <ArtistCard obj={obj} index={i} />)
        ) : (
