...
const Donate = ({ layoutData, pageData }) => {
    const quote = parseMarkdown(
        pageData.content?.find((el) => el.area === "quote").content
    )
    const rightCol = parseMarkdown(
        pageData.content?.find((el) => el.area === "rightCol").content
    )

    // add in Blockquote class
    quote.children[0].properties = Object.assign(
        quote.children[0].properties || {},
        { class: "quote_graphic-bar" }
    )
