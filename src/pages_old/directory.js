...
const SearchButton = () => (
    <button
        className="btn bg_slate btn_search"
        onClick={() =>
            window.scrollTo({
                top:
...
const Directory = ({ data }) => {
    let searchList = data.allDatoCmsKey.edges.filter(
        ({ node }) => node.showInDirectory
    )
    searchList = searchList.sort((a, b) => {
        ...
        if (
            aNameToken[aNameToken.length - 1] <
            bNameToken[bNameToken.length - 1]
        ) {
            return -1
        }
        if (
    ...
    searchList.forEach(({ node }) => {
        ...
        node.lastName = node.name.substr(
            node.name.indexOf(" ") + 1,
            node.name.length
        )
    })
    ...
    filters.forEach((filter) => {
        ...
        if (filter.type === "dropdown") {
            filter.values = searchList
                .map(({ node }) => node[filter.field])
                .filter((val, i) => val)
            filter.values = [...new Set(filter.values)].sort()
    ...
    globalSearchConfig.keys = Object.keys(searchList[0].node).map(
        (key) => `node.${key}`
    )
    ...
    const localStorageFilters =
        typeof window !== "undefined"
            ? localStorage.getItem("latestFilters")
            : undefined
    ...
    useEffect(() => {
        async function updateSearchResults() {
            ...
            orFilters.forEach((filter) => {
                ...
                filter.results.forEach((filterRes) => {
                    if (
                        !finalResults.find(
                            (finalRes) =>
                                finalRes.node.name === filterRes.node.name
                        )
                    ) {
            ...
            andFilters.forEach((filter) => {
                if (!filter || filter.results.length === 0) return
                finalResults = finalResults.filter((finalRes) =>
                    filter.results.find(
                        (filterRes) =>
                            finalRes.node.name === filterRes.node.name
                    )
            ...
            const filtersAreActive = Object.keys(formik.values).some(
                (field) => {
        ...
        for (const filter of filters) {
            if (formik.values[filter.field]) {
                if (filter.type === "checkbox") {
                    const trueVals = filter.values.filter(
                        (val, i) => formik.values[filter.field][i]
                    )

                    filter.results = searchList.filter(
                        ({ node }) =>
                            node[filter.field] &&
                            trueVals[0] &&
                            trueVals.some((val) =>
                    ...
                    filter.results = filter.fuse
                        .search(formik.values[filter.field])
    ...
    const activeFilters = filters.filter((f) =>
        f.type !== "checkbox"
            ? formik.values[f.field]
            : formik.values[f.field].some((val) => val)
    ).length

    return (
        <Layout
            classNames={["fullwidth", "directory"]}
    ...
    artists."
            footerQuoteText={
                <blockquote>"You have to see it to be it."</blockquote>
            }
            footerQuoteAttribution="Jeanine Tesori"
            footerQuoteBgColor="var(--rok-pale-green-1_hex)"
        >
            <section className="section_intro">
                <h1>Directory</h1>
                <p>
                    We have curated a directory of actors, directors, dancers,
                    singers, stage managers, lighting designers, dramaturgs,
                    artistic directors, producers, casting directors,
                    librettists, lyricists, composers, props designers, scenic
                    designers, sound designers, choreographers, costume
                    designers, and production managers who self-identify as
                    lesbian, bisexual, trans, queer, femme, masc, non-binary,
                    and the diversity of genders that queerness contains.
                </p>
            </section>
            <section className="section_search">
                <div className="input__group text">
                    <label htmlFor="fuzzy">
                        Search any keywords here or use the advanced search
                ...
                <button
                    ...
                    className={
                        "advanced-btn " + (activeFilters ? "active" : "")
                    }
                    style={{
            ...
            <section
                className={`section_filters ${
                    filtersAreVisible ? "active" : ""
                }`}
            >
                <div className="filters">
                    <button
                        ...
                        onClick={() =>
                            document.querySelector("a.key__card").focus()
                        }
                    >
            ...
            <section id="key__grid" className="key__grid">
                {searchResults[0] ? (
                    searchResults.map((obj, i, arr) => (
                        <ArtistCard obj={obj} index={i} />
                    ))
                ) : (
...
async function resetFilters(formik, filters, postCallback) {
    Array.from(document.querySelectorAll('.filters [type="checkbox"]')).forEach(
        (input) => {
            input.checked = false
        }
    )
