...
const Resources = ({ data }) => {
  ...
  const resources = data.allDatoCmsResource.edges.reduce(
    (acc, { node }) => {
      if (!resourceTypes.find(el => el === node.resourceType)) {
        ...
        acc[
          acc.findIndex(resource => resource.type === node.resourceType)
        ].resources.push(node)
      }
      return acc
    },
  ...
  const numResources =
    resources && resources.reduce((acc, r) => acc + r.resources.length, 0)
  ...
  return (
    <Layout
      ...
      description={`A curated list of ${
        approxNumResources ? approxNumResources : 50
      }+ queer resources for theatremakers.`}
      classNames={["fullwidth"]}
    >
      <div className="resources-intro">
        <h1>Resources</h1>
        {renderHtmlToReact(
          data.pageContent.bodyNode.childMarkdownRemark.htmlAst
        )}
      </div>
      {resources &&
        resources.map(({ type, resources: resourceList }, i) => (
          <Carousel
            classNames={["resource-carousel"]}
            style={{
              "--theme-color": resourceThemes[i % resourceThemes.length],
            }}
          >
            <div
      ...
      <section className="resource-annotation">
        <img
          src={devonImg}
          alt="Devon Hayawaka"
          style={{ width: "70px", marginRight: "1.2rem" }}
        />
        This list was compiled by Key Member&nbsp;
        <Link to="/keys/devon-hayakawa">Devon Hayakawa</Link>, a Chicago-based
        Performer, playwright, and dramaturg.
      </section>
    </Layout>
  )
}
