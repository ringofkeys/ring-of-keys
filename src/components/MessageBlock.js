...
const MessageBlock = ({ messages }) => {
    ...
    <section className='block block_messages'>
        ...
        <div ariaRole='button' className={ 'message ' + (parseInt(message.meta.timeDiff) < 5 ? 'recent' : '') } onClick={ () => {
            setPopupIndex(i)
            setPopupOpen(true)
        } } style={{ '--time-string': message.meta.timeString }}>
            <div className='message_labels'>
                <h3>{ message.fromName }</h3>
                <p><em>{ message.meta.timeString }</em></p>
            </div>
            <p>{ (message.message.length > messagePreviewLength) ? (message.message.substr(0, messagePreviewLength) + (message.message[messagePreviewLength] === '.' ? '' : '...')) : message.message }</p>
