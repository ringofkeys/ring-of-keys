...
const HeroSocialMediaEditor = ({
    isOpen = false,
    onClose = () => {
        console.log("closing hero editor!")
    },
}) => {
    const [isSubmitting, setSubmitting] = useState(false)
    const {
        artist: { id, socialMedia },
        artistDispatch,
    ...
    return (
        <Popup isOpen={isOpen} onClose={onClose}>
            ...
            <p>
                Go to your social media profile and copy &amp; paste the URL
                from the browser in the appropriate field
            </p>
            <form id="edit-social-media" onSubmit={handleSubmit}>
                {Object.keys(socialIcons).map((key) => {
                    const hasLink = socialMedia.find((socialObj) =>
                    ...
                    return (
                        <FormField
                            ...
                            key={key}
                            defaultValue={
                                hasLink ? hasLink.socialMediaLink : ""
                            }
                        />
                    )
                })}
                <div className={popupStyles.buttonRow}>
                    <button className="btn btn-link_ghost" onClick={onClose}>
                        Cancel
                    </button>
                    <button
                        type="submit"
