...
function HeroHeadshotEditor({
    ...
    const { artist, artistDispatch } = useContext(ProfileContext)
    ...
    async function handleSubmit(e) {
        ...
        const path = await client.createUploadPath(file, {
            filename:
                toKebabCase(artist?.name) + file.type.replace("image/", ""),
            onProgress: (event) => {
    ...
    return (
        <Popup isOpen={isOpen} onClose={onClose}>
            ...
            <form id="edit-headshot" onSubmit={handleSubmit}>
                {!isSubmitting ? (
...
function buildHeadshotObj(headshot) {
    return {
        responsiveImage: {
            src:
                headshot.url +
                "?auto=format&facepad=50&fit=facearea&h=300&w=300",
            width: 300,
            height: 300,
            aspectRatio: 1,
