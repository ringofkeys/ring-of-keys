...
const Sidebar = () => {
    const data = useStaticQuery(graphql`
        query SidebarQuery {
            ...
            team: allDatoCmsKey(filter: { keyTeamMember: { eq: true } }) {
                edges {
                    node {
                        name
                        slug
    ...
    return (
        <aside className="sidebar">
            ...
            {data.team.edges
                .sort((a, b) => a.node.keyTeamOrder - b.node.keyTeamOrder)
                .map(({ node }) => (
                    <Link
                        to={`/keys/${node.slug}`}
                        className="teammate"
                        key={node.slug}
                    >
                        <strong>{node.name}</strong> <em>({node.pronouns})</em>
                        <br />
                        <em style={{ color: "#6d7278" }}>
                            {node.keyTeamPosition}
                        </em>
                    </Link>
                ))}
            <h3>Meetup Ambassadors</h3>
            {data.ambassadors.edges
                .sort(
                    (a, b) =>
                        a.node.meetupAmbassadorOrder -
                        b.node.meetupAmbassadorOrder
                )
                .map(({ node }) => (
                    <Link
                        to={`/keys/${node.slug}`}
                    ...
                    >
                        ...
                        {node.pronouns.toLowerCase()}) <br />
                        <em style={{ color: "#6d7278" }}>
                            {getBestLocationVal(node)}
                        </em>
                    </Link>
                ))}
        </aside>
