...
function cssModularize(cssClassString) {
    cssClassString.split(' ').map(c => styles[c]).join(' ')
}

const CarouselCardInner = ({ node, recordType, ratio, className }) => {
    function wrapLink(node, className, children) {
        return node.externalUrl 
            ?   <a href={node.externalUrl} rel='noopener noreferrer' target='_blank' className={cssModularize(className)} tabIndex='-1'>{ children }</a>
            :   <Link href={(recordType ? '/'+recordType : '') + '/' + (node.slug ? node.slug : '#')} tabIndex='-1'>
    ...
    function truncatedDescription(descriptionHTML) {
        ...
        const truncatedPara = descriptionObjects.find(d => typeof d == 'string' || (d.type == 'p' && typeof d.props.children == 'string'))
        if (!truncatedPara) return
        return ((typeof truncatedPara == 'string')
            ? truncatedPara
            : truncatedPara.props.children).slice(0, TRUNCATE_LENGTH) + '...'
    }

    return (
    <li className={`${styles['carousel_card']} ${styles['hover_scale']} ${className}`} style={{ opacity: ratio ? ratio : 1 }}>
            ...
            <div>
                ...
                { (node.body || node.description) && truncatedDescription(node.body ? node.body : node.description) }
            </div>
            { node.externalUrl 
            ? <a href={ node.externalUrl } className='btn btn-link_ghost' rel='noopener noreferrer' target='_blank'>Read More</a>
            : <Link href={ (recordType ? '/'+recordType : '') + '/' + (node.slug ? node.slug : '#') } className='btn btn-link_ghost'>
                <a>Read More</a></Link> }
        </li>
    )
}
