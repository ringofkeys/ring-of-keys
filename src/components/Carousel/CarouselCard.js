...
const CarouselCardInner = ({ node, recordType, ratio, className }) => {
    function wrapLink(node, className, children) {
        return node.externalUrl 
            ...
            :   <Link href={(recordType ? '/'+recordType : '') + '/' + (node.slug ? node.slug : '#')} className={cssModularize(className)} tabIndex='-1'>
                    <a>{ children }</a>
                </Link>
    }
    ...
    <li className={`${styles['carousel_card']} ${styles['hover_scale']} ${className}`} style={{ opacity: ratio ? ratio : 1 }}>
            ...
            <div>
                ...
                {/* { (node.bodyNode || node.desscriptionNode) ? renderHtmlToReact(node.bodyNode ? node.bodyNode.childMarkdownRemark.excerptAst : node.descriptionNode.childMarkdownRemark.excerptAst) : '' } */}
            </div>
            { node.externalUrl 
