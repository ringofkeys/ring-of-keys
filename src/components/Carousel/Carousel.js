...
const Carousel = ({ heading, recordType, itemList, classNames = [], style, children }) => {
    ...
    return (<section className={styles['carousel-section']} style={Object.assign({position: 'relative', paddingBlockStart: '1.5rem'}, (style) ? style : {})}>
        <div className={styles['carousel-title']}>
            { heading && (
                <h2>{ heading }</h2>
            )}
            <ul className={styles['carousel_advance_btns']} style={{top: (!heading) ? '0': '2.4rem' }}>
                <li>
        ...
        <div ref={ carousel } tabIndex='0' className={`${styles['carousel']} ${ classNames.length > 0 ? classNames.join(' ') : '' }`}>
