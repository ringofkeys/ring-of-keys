...
const CarouselCardInner = ({ node, recordType, ratio, className }) => {
  ...
  return (
    <li
      className={"carousel_card hover_scale " + className}
    ...
    >
      ...
      <div>
        ...
        {node.bodyNode || node.descriptionNode
          ? renderHtmlToReact(
              node.bodyNode
                ? node.bodyNode.childMarkdownRemark.excerptAst
                : node.descriptionNode.childMarkdownRemark.excerptAst
            )
          : ""}
      </div>
      {node.externalUrl ? (
