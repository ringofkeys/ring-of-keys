...
export default function FormField({
    ...
    const client = buildClient({
        apiToken: process.env.NEXT_PUBLIC_DATO_READ_WRITE_TOKEN,
    })
    ...
    function validateFileSize(e) {
        if (e.target.files[0]) {
            ...
            if (file.size > 4000000) {
                ...
                const errMessage = `This file is ${(
                    file.size / 1_000_000
                ).toFixed(1)}Mb, please upload one smaller than 4Mb.`
    ...
    return (
        <div className={styles.field + " file " + (className ? className : "")}>
            {label && (
                <label
                    htmlFor={name}
