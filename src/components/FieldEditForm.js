...
const FieldEditForm = ({
  ...
  <div id={id} className={"profile_field_group " + type}>
    ...
    >
      ...
      {type === "checkbox" && (
        <CheckboxGrid label={label} helpText={helpText} fieldData={field.data}>
          {field.refArray.map((f, i) => (
            <Field
              type="checkbox"
              name={`${field.fieldName}[${i}]`}
              label={f}
              key={f}
              defaultChecked={field.data[i]}
            />
          ))}
          <Field
            type="text"
            name={`${field.fieldName}-Other`}
            label="Other"
      ...
      <button
        type="submit"
        className={`btn bg_slate btn_submit ${
