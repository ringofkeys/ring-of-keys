...
const ResourceType = ({ pageContext: context }) => {
  return (
    <Layout
      title={`Resources - ${context.resourceType}`}
      description={`We've compiled a list of ${
        context.resources.length
      } ${context.resourceType.toLowerCase()} resources for LGBT+ theatremakers to access.`}
    >
        ...
        {context.resources &&
          context.resources.map(resource => (
            <ResourceCard
              ...
              href={resource.link}
              color={
                resourceThemes[context.typeIndex % (resourceThemes.length - 1)]
              }
            />
          ))}
      </div>
