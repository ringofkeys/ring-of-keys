...
import { fieldArrayToString } from "../../utils/profileEditor"

const BasicInfoField = ({ field, index, editorState, userId }) => {
  return (
    <>
      {!(field.type === "checkbox" && field.isEditing) && (
        <h3>
          {field.label}
          ...
          <p>
            {!field.data.includes("http") ? (
              field.data ? (
                ...
                <span className="unfilled-field">Add some info here!</span>
              )
            ) : (
              <a
              ...
              >
                {field.data ? (
                  ...
                  <span className="unfilled-field">Add a URL here!</span>
                )}
              </a>
            )}
          </p>
