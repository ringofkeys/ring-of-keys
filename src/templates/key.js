...
export default ({ data }) => {
  ...
  const isProfileOwner =
    isAuthenticated() &&
    userProfile &&
      userProfile["https://ringofkeys.org/user_metadata"].entity_id ===
        data.datoCmsKey.id.match(/-(\d+)-/)[1]
  ...
  const applyDatoField = field => {
    field.data = !field.initialVals
      ? data.datoCmsKey[field.fieldName]
      : [...field.initialVals, field.initialOther]
    return field
  }

  const heroFields = getHeroFields()
  Object.keys(heroFields).forEach(key => {
    heroFields[key].data = data.datoCmsKey[heroFields[key].fieldName]
  })
  ...
  useEffect(() => {
    ...
    heroFields["headshot"].data.url +=
      "?fit=facearea&faceindex=1&facepad=5&mask=ellipse&w=180&h=180&"
  }, [])
  ...
  const bodyFields = getBodyFields(
    data.datoCmsKey.affiliations,
    data.datoCmsKey.locations
  ).map(applyDatoField)
  ...
  const infoFields = getInfoFields(data.datoCmsKey.locations).map(
    applyDatoField
  )
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="avatar">
            {!isEditable ? (
              <img
                src={heroFields.headshot.data.url}
                alt={heroFields.headshot.data.title}
                className="headshot"
              ...
              <div className="headshot_group">
                ...
                />
                <button
                  className="btn_edit edit_headshot"
                  onClick={() => heroFields.headshot.setEditing(true)}
                >
                  <img
          ...
          <div className="artist_bio">
            ...
            {infoFields[2].data && (
              <p>Based in {infoFields[2].data.replace(", ", " • ")}</p>
            )}
            <p>
              {infoFields[1].data}
              {data.datoCmsKey.memberSince
                ? ` • Member Since ${data.datoCmsKey.memberSince}`
                : ""}
            </p>
            {isEditable && (
            ...
            {!isEditable ? (
              heroFields.socialMedia.data && (
                <>
                  {heroFields.socialMedia.data.map(socialObj => {
                    const mediaPlatform = Object.keys(socialIcons).filter(key =>
                      socialObj.socialMediaLink.includes(key)
                    )[0]
                    return (
                      <a
                        href={socialObj.socialMediaLink}
              ...
              <>
                {Object.keys(socialIcons).map(key => {
                  const hasLink = heroFields.socialMedia.data.find(socialObj =>
                    socialObj.socialMediaLink.includes(key)
                  )
                  return (
                ...
                <button
                  ...
                  onClick={() => heroFields.socialMedia.setEditing(true)}
                >
                  <img
          ...
          {heroFields.featuredImage.data && (
            <img
              src={heroFields.featuredImage.data.url}
              alt={heroFields.featuredImage.data.alt}
          ...
          {isEditable && (
            <button
              ...
              onClick={() => heroFields.featuredImage.setEditing(true)}
            >
              <img
                src={profileIcons.camera}
                className="icon_edit"
          ...
          <Link to="/directory" className="back_link">
      ...
      {hasSubmitted && <PreviewMessage />}
      <MessagePopup
        isOpen={isMessageOpen}
        artistId={data.datoCmsKey.id}
        artistName={data.datoCmsKey.name}
        onClose={() => setMessageOpen(false)}
      />
      {heroFields.headshot.isEditing && (
        <Hero.HeroHeadshotImageEditor
          userId={data.datoCmsKey.id}
          field={heroFields.headshot}
          editorState={editorState}
        />
      )}
      {heroFields.featuredImage.isEditing && (
        <Hero.HeroFeaturedImageEditor
          userId={data.datoCmsKey.id}
          field={heroFields.featuredImage}
          editorState={editorState}
        />
      )}
      {heroFields.socialMedia.isEditing && (
        <Hero.HeroSocialMediaEditor
          userId={data.datoCmsKey.id}
          field={heroFields.socialMedia}
          editorState={editorState}
        />
      )}
      <EmailPopup />
      ...
      >
        <img
          src={heroFields["headshot"].data.fullUrl}
          alt={`${infoFields[0].data} headshot`}
        />
      </Popup>
    </>
  )
...
function getInfoFields(locations) {
  return [
    ...
    {
      label: "Regions",
      helpText: "(These control your search page listing)",
      refArray: locationLabels(),
      fieldName: "locations",
      type: "checkbox",
...
function getBodyFields(affiliations) {
  return [
    ...
    { label: "Dance Experience", fieldName: "danceExperience", type: "text" },
    {
      ...
      refArray: affiliationLabels(),
      fieldName: "affiliations",
      type: "checkbox",
      initialVals: affiliationLabels().map(label =>
        affiliations.includes(label)
      ),
...
function getInitialOther(string, valArray, splitStr) {
  const filteredOther = string
    .split(splitStr)
    .filter(val => valArray.indexOf(val) < 0)
