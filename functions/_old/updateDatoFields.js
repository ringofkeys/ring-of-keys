...
exports.handler = async (event, context, callback) => {
    if (event.httpMethod === "OPTIONS") {
        ...
        try {
            ...
            const fileFildePromises = await Promise.all(
                fileFields.map(async (fileField) => {
                    if (!data[fileField]) return

                    return await client.uploads
                        .create({
                            path: data[fileField],
                            author: `${name ? name : id} via profile editor`,
                            defaultFieldMetadata: {
                                en: {
                                    alt: `editor-uploaded image of ${
                                        name ? name : id
                                    }`,
                                    title: `${
                                        name ? name : id
                                    }, ${fileField} via Editor, ${
            ...
            if (data.socialMedia) {
                data.socialMedia = data.socialMedia.map(({ socialMediaLink }) =>
                    buildModularBlock({
                        socialMediaLink,
                        itemType: "181488",
                    })
            ...
            const updateRes = await client.items
                .update(id, data)
                .catch((err) => console.error(err))
            ...
            callback(null, {
                ...
                body: JSON.stringify(
                    newName && nameChangeSuccessful
                        ? { nameChangeSuccessful, updateRes }
                        : updateRes
                ),
            })
        } catch (err) {
...
async function getAuth0Token() {
    ...
    return await fetch(
        `https://${process.env.GATSBY_AUTH0_DOMAIN}/oauth/token`,
        options
    )
        .then((res) => res.json())
        .catch((err) => console.error(err))
...
async function getAuth0UserID(auth, id) {
    ...
    return await fetch(
        `https://${process.env.GATSBY_AUTH0_DOMAIN}/api/v2/users?q=user_metadata.entity_id%3A%22${id}%22&fields=user_id&include_fields=true&search_engine=v3`,
        options
    )
        .then((res) => res.json())
        .catch((err) => console.error(err))
...
async function updateAuth0Name(auth, id, name) {
    ...
    return await fetch(
        `https://${process.env.GATSBY_AUTH0_DOMAIN}/api/v2/users/${id}`,
        options
    )
        .then((res) => res.json())
        .catch((err) => console.error(err))
